...
{
    // 500 MB upload cap for match videos.
    ...
    [HttpGet("clubs/{clubId:guid}/teams/{teamId:guid}/events/{eventId:guid}/videos")]
    public async Task<IActionResult> GetVideos(Guid clubId, Guid teamId, Guid eventId)
    {
        ...
        var rows = await _db.GameVideos
            .Where(v => v.EventId == eventId && v.TeamId == teamId)
        ...
        var result = rows.Select(v => new
        {
            ...
            v.ContentType,
            v.FileSize,
            StreamPath = StreamPathFor(clubId, teamId, eventId, v.VideoId),
            v.AddedByUserId,
            v.AddedByName,
            v.AddedByRole,
            v.CanEdit,
    ...
    [HttpPost("clubs/{clubId:guid}/teams/{teamId:guid}/events/{eventId:guid}/videos")]
        ...
        [FromForm] IFormFile? file, [FromForm] string? title)
    {
        ...
        if (string.IsNullOrEmpty(cleanTitle))
            cleanTitle = "Game video";

        var now = DateTime.UtcNow;
        var video = new GameVideo
        {
        ...
        _db.GameVideos.Add(video);
        await _db.SaveChangesAsync();

        return Ok(await ProjectVideoAsync(video.VideoId, clubId, teamId, eventId, userId.Value));
    }

    [HttpGet("clubs/{clubId:guid}/teams/{teamId:guid}/events/{eventId:guid}/videos/{videoId:guid}/stream")]
    public async Task<IActionResult> StreamVideo(Guid clubId, Guid teamId, Guid eventId, Guid videoId)
    {
        var userId = GetCallerUserId();
        if (userId == null) return Unauthorized(new { error = "Invalid token." });
        if (!await CanViewTeamAsync(clubId, teamId, userId.Value))
        ...
        var video = await _db.GameVideos
    ...
    public async Task<IActionResult> DeleteVideo(Guid clubId, Guid teamId, Guid eventId, Guid videoId)
    {
        var userId = GetCallerUserId();
        if (userId == null) return Unauthorized(new { error = "Invalid token." });
    ...
    private async Task<object?> ProjectVideoAsync(
        Guid videoId, Guid clubId, Guid teamId, Guid eventId, Guid callerUserId)
    {
        var v = await _db.GameVideos
    ...
    private static string StreamPathFor(Guid clubId, Guid teamId, Guid eventId, Guid videoId) =>
        $"/clubs/{clubId}/teams/{teamId}/events/{eventId}/videos/{videoId}/stream";
